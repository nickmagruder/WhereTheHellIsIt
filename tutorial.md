...
With Docker running, we can define the full database schema. Drizzle ORM uses TypeScript files as the schema source of truth — `drizzle-kit generate` reads them and produces migration SQL.

**Files created (in FK dependency order):**

```text
packages/db/src/schema/
  users.ts         — no FKs
  spaces.ts        — FK → users
  space_members.ts — FK → spaces, users
  locations.ts     — FK → spaces; ltree column (customType)
  items.ts         — FK → spaces, locations, users; tsvector GENERATED column
  photos.ts        — FK → items, users
  index.ts         — re-exports all six tables
```

`packages/db/src/index.ts` re-exports from `schema/index.ts` so the package has a clean public API:

```typescript
export * from './schema';
```

#### users.ts — simple, no FKs
...
All PKs are `uuid().defaultRandom()` — UUIDs generated by PostgreSQL (`gen_random_uuid()`). This avoids sequential IDs leaking counts and simplifies distributed inserts.

#### space_members.ts — pgEnum for role

Role is defined as a **pgEnum**, not a `CHECK` constraint:

```typescript
export const roleEnum = pgEnum('role', ['owner', 'editor', 'viewer']);
```

The difference matters: a PostgreSQL enum type is a first-class DB type that appears in `pg_type`. A CHECK constraint is just a column-level validation. The enum approach lets Drizzle infer the TypeScript union type automatically (`'owner' | 'editor' | 'viewer'`) and makes invalid values impossible at the database level, not just at the application level.

The table also gets a composite unique constraint and an index:
...
The third argument to `pgTable` in drizzle-orm 0.36 is an **array** of constraint/index builders (not an object as in earlier versions).

#### locations.ts — customType for ltree

`ltree` is a PostgreSQL extension type — there's no built-in Drizzle column type for it. `customType` from `drizzle-orm/pg-core` lets us define arbitrary column types:
...
This tells Drizzle to use `ltree` as the SQL column type and `string` as the TypeScript type. The GIST index uses `using` with the index method and column together:

```typescript
index('locations_path_idx').using('gist', table.path)
```

**Gotcha: self-referential FK causes TypeScript circular type inference.** `locations.parent_id` should reference `locations.id`, but writing `.references(() => locations.id)` makes TypeScript unable to infer the table's type — it's used in its own initializer. The workaround: define `parentId` as a plain `uuid('parent_id')` and add the FK constraint (`parent_id → locations(id) ON DELETE SET NULL`) manually in the migration SQL.

#### items.ts — tsvector GENERATED column and GIN indexes

The `search_vector` column is a PostgreSQL `GENERATED ALWAYS AS STORED` column — PostgreSQL maintains it automatically from `name`, `description`, and `tags`. We use two Drizzle features together:

**1. customType for tsvector:**
...
**2. generatedAlwaysAs for the expression:**
...
Note: in drizzle-orm 0.36, `generatedAlwaysAs` takes a single SQL expression argument (no `{ mode: 'stored' }` options object — stored mode is the default). Drizzle excludes generated columns from insert/update TypeScript types, so you can't accidentally try to set `search_vector` on insert.

**GIN indexes for search and tags:**
...
The `tags` column is a PostgreSQL text array (`text('tags').array()`) with a GIN index — this enables efficient `@>` containment queries ("find all items that have all of these tags").

**Circular FK with photos:** `items.primary_photo_id` references `photos.id`, but `photos.item_id` also references `items.id`. This creates a circular import if both tables use `.references()`. The solution: define `primaryPhotoId` as a plain `uuid` column and add the FK constraint in migration SQL after both tables are created.
...
This file lives at `packages/db/drizzle.config.ts` (not in `src/` — it's a drizzle-kit config file, not application code). The `tsconfig.json` only includes `src/**/*`, so this file is not type-checked by the package's tsc run. That's intentional — drizzle-kit runs it directly with its own loader.
...
```json
"db:generate": "pnpm --filter=@wthi/db db:generate",
"db:migrate": "pnpm --filter=@wthi/db db:migrate",
"db:studio": "pnpm --filter=@wthi/db db:studio",
...
# Open Drizzle Studio (web UI to browse the schema and data)
pnpm db:studio
```

#### Type-checking the schema

After writing all six schema files, run:

```bash
pnpm --filter=@wthi/db type-check
```

All 24 turbo tasks (`lint` + `type-check` across all packages) pass with `pnpm turbo lint type-check`.

> **Next up:** Step 1.3 — First Migration (generate SQL, add extension setup, apply to Docker PostgreSQL).
